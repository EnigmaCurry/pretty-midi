...
class PrettyMIDI(object):
    ...
    def adjust_times(self, original_times, new_times):
        ...
        if (unique_idx.size != original_size) or any(
        ...
        if not np.all(np.diff(new_times) >= 0):
            warnings.warn(
                "new_times must be monotonic; " "automatically enforcing this."
            )
    ...
    def write(self, filename):
        ...
        for l in self.text_events:
            timing_track.append(
                mido.MetaMessage("text", time=self.time_to_tick(l.time), text=l.text)
            )
